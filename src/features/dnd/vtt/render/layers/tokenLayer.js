...
export class TokenLayer {
  ...
  update(tokens, grid, selectedIds, draggingId, hiddenTokens, dmView = false, viewerId = null, bloody = false, elevations = null, turn = null, badgeScale = 1, bloodHp = null) {
    this._dmView = dmView; this._viewerId = viewerId; this._bloody = bloody; this._elev = elevations || {}; this._bloodHp = bloodHp || null;
  ...
  tickTokens(skipId) {
    for (const [id, node] of this.nodes) {
      if (id === skipId || node._tx == null) continue;
      const p = node.root.position;
      const dx = node._tx - p.x, dy = node._ty - p.y;
      if (Math.abs(dx) < 0.4 && Math.abs(dy) < 0.4) { if (dx || dy) p.set(node._tx, node._ty); continue; }
      p.set(p.x + dx * 0.3, p.y + dy * 0.3);
    }
  }
  ...
  create(t) {
    ...
    // order: selection, disc, portrait, blood overlay, masks, initial, ring, hp, badges, ac, elev
    root.addChild(sel, disc, portrait, blood, mask, bloodMask, initial, ring, hp, badges, ac, elev);
    let lastTap = 0;
    ...
    const node = { root, disc, ring, portrait, blood, mask, bloodMask, initial, hp, badges, sel, ac, elev, url: null, bloodKey: null };
  ...
  draw(node, t, grid, selected, dmView) {
    ...
    this.drawAc(node.ac, (dmView && t.kind === 'npc' && t.ac != null) ? t.ac : null, r, sc);
    ...
    if (showHp && t.hp != null && t.hpMax) {
      const w = r * 1.8, h = Math.max(4, r * 0.12 * sc);
      const x = -w / 2, y = r + 4;
    ...
    this.drawBadges(node.badges, t, r, sc);
  ...
  drawAc(container, ac, r, sc = 1) {
    container.removeChildren().forEach((c) => c.destroy({ children: true }));
    if (ac == null) { container.visible = false; return; }
    container.visible = true;
    // Proportional to the token (tiny floor only) so it isn't oversized on
    // small-grid / 1-cell tokens and actually responds to the badge-size slider.
    const br = Math.max(3, r * 0.16 * sc);
  ...
  drawBadges(badges, t, r, sc = 1) {
    badges.removeChildren().forEach((c) => c.destroy({ children: true }));
    const ids = t.conditions || [];
    const br = Math.max(4, r * 0.22 * sc); // badge radius (small floor → scales with token + slider)
