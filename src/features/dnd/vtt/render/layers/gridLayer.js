...
export class GridLayer {
  ...
  update(map) {
    ...
    const g = map.grid;
    const key = `${map.width}x${map.height}|${g.size}|${g.sizeX}|${g.sizeY}|${g.offsetX}|${g.offsetY}|${g.color}|${g.opacity}|${g.thickness}|${g.style}|${g.snapMapToGrid ? `${g.cols}x${g.rows}` : ''}`;
  ...
  draw(map) {
    ...
    // "Auf volle Felder snappen": genau g.cols × g.rows Zellen mit den
    // per-Achsen-Zellgrößen (sizeX/sizeY) — das Raster füllt die Karte EXAKT
    // (cols·sizeX == mapWidth, rows·sizeY == mapHeight), keine halben Felder.
    if (g.snapMapToGrid && g.cols > 0 && g.rows > 0) {
      const sx = g.sizeX || g.size; const sy = g.sizeY || g.size;
      const x0 = g.offsetX; const x1 = g.offsetX + g.cols * sx;
      const y0 = g.offsetY; const y1 = g.offsetY + g.rows * sy;
      for (let c = 0; c <= g.cols; c++) addLine(gfx, x0 + c * sx, y0, x0 + c * sx, y1, dash);
      for (let r = 0; r <= g.rows; r++) addLine(gfx, x0, y0 + r * sy, x1, y0 + r * sy, dash);
