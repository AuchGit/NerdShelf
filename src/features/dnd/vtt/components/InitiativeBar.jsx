        ...
        {init.order.map((e, i) => {
          ...
          return (
            <div key={e.id} onClick={() => t && selectToken(e.tokenId)}
              style={{ ...S.item, ...bg, ...(active ? S.active : next ? S.next : null) }}
              title={active ? `${e.name} — am Zug` : next ? `${e.name} — als Nächstes` : e.name}>
              {!t?.imageUrl && <div style={S.initial}>{(e.name || '?').slice(0, 1).toUpperCase()}</div>}
              {next && <div style={S.nextTag}>nächste</div>}
...
const S = {
  ...
  next: { border: '2px dashed var(--color-warning,#e0af68)', boxShadow: '0 0 8px -2px var(--color-warning,#e0af68)' },
  nextTag: { position: 'absolute', top: 2, left: 2, fontSize: 8, fontWeight: 800, color: '#1a1a1a', background: 'var(--color-warning,#e0af68)', borderRadius: 4, padding: '0 4px', textTransform: 'uppercase' },
